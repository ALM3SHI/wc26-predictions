  ...
  if (!scorers.length) {
    return (
      <div className="rounded-3xl border border-gray-200 bg-white p-6 text-center text-sm text-gray-500">
        Golden Boot race hasn&apos;t opened yet. Check back after Matchday 1.
      </div>
    );
  }
    ...
    <div className="rounded-3xl border border-gray-200 bg-white p-4 md:p-6 overflow-hidden relative">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Live — updated from football-data.org
          </p>
        </div>
      </div>
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] uppercase tracking-widest text-gray-400 truncate">
                  {s.team.shortName} · age {ageFrom(s.player.dateOfBirth)}
                </div>
              </div>
              <div className="text-[10px] text-gray-400 hidden md:block">
                A: <span className="font-bold text-gray-700">{s.assists ?? 0}</span>
              </div>
              <div className="text-[10px] text-gray-400 hidden md:block">
                P: <span className="font-bold text-gray-700">{s.penalties ?? 0}</span>
              </div>
              <div className="font-fifa text-xl text-gray-900 min-w-[36px] text-right">
                {s.goals}
              </div>
            </motion.div>
          ))}
      ...
      <div className="mt-4 flex items-center justify-between text-[10px] uppercase tracking-widest text-gray-400">
        <div className="flex items-center gap-1">
          <Award className="w-3 h-3" /> Live table
        </div>
